...
describe('RelationManager — validation', () => {
	it('rejects an undeclared relation target at construction', () => {
		const db = createDatabase({
			driver: createMemoryDriver(),
			tables: { accounts: { id: stringShape(), name: stringShape() } },
		})
	...
	it('rejects an undeclared through junction at construction', () => {
		const db = createDatabase({
			driver: createMemoryDriver(),
			tables: {
				accounts: { id: stringShape(), name: stringShape() },
		...
		expect(() =>
			createRelationManager({
				database: db,
				relations: {
					accounts: { reps: hasThrough('missing', 'accountId', 'repId', 'reps') },
				},
			}),
		).toThrow(
			expect.objectContaining({
				code: 'INVALID',
				message: expect.stringMatching(/accounts.*reps.*missing/),
			}),
		)
	})
})
